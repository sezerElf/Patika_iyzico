...
const allFoods = () => {
  let food ="";
  menu.map((foods) => {
    food += createFood(foods);
  });
  menuSection.innerHTML= food;
};
...
const koreanFood = ()=>{
  let koreanFood  ="";
  menu.map((korean)=>{
    if(korean.category==="Korea"){
      koreanFood += createFood(korean);
    }
  })
  menuSection.innerHTML = koreanFood;
  console.log(koreanFood);
}
let koreanDom =document.querySelector("#korea");
koreanDom.addEventListener("click",koreanFood);

//list japan food
const japanFood = ()=> {
  let japanFoods = "";
  menu.map((japan)=>{
    if(japan.category==="Japan"){
      japanFoods += createFood(japan);
    }
  })
  menuSection.innerHTML = japanFoods;
  console.log(japanFoods);
}
let japanDom = document.querySelector("#japan");
japanDom.addEventListener("click",japanFood);

//list china foods
const chinaFood = () => {
  let chinaFoods="";
  menu.map((china)=>{
    if(china.category==="China"){
      chinaFoods += createFood(china)
    }
  })
  menuSection.innerHTML = chinaFoods;
}
let chinaDom = document.querySelector("#china");
chinaDom.addEventListener("click",chinaFood);
